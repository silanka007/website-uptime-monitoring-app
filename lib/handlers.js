...
handlers._users.delete = function(data, callback){
    ...
    if(queryPhone){
        ...
        handlers._tokens.verifyToken(token, queryPhone, function(tokenIsValid){
            if(tokenIsValid){
                _data.read('users', queryPhone, function(err, userData){
                    if(!err && userData){
                        _data.delete('users', queryPhone, function(err){
                            if(!err){
                                //deleting all the checks associated to a user
                                    ...
                                    userChecks.forEach(function(checkId){
                                        _data.delete('checks', checkId, function(err){
                                            ...
                                            if(checksDeleted == checksToDelete){
                                                if(deletionError){
                                                    callback(500, {'error' : 'error deleting associated checks. not all user checks deleted'})
                                                }else{
                                                    callback(200);
                                                }
                                            }
                ...
                callback(400, {'error' : 'invalid token provided - please provide a valid token'})
            }
        })
    }else{
        callback(404, {'error' : 'missing required field'})
                    ...
                    handlers._tokens.verifyToken(token, userPhone, function(tokenIsValid){
                        if(tokenIsValid){
                            ...
                            _data.read('users', userPhone, function(err, userData){
                                if(!err && userData){
                                    const checks = typeof userData.checks === 'object' && userData.checks instanceof Array && userData.checks.length > 0 ? userData.checks : [];
                            ...
                            if(tokenIsValid){
                                ...
                                _data.read('users', userPhone, function(err, userData){
                                    if(!err && userData){
                                        ...
                                        callback(500, {'error' : 'unable to get data'});
                                    }
                                })
                            }else{
...
handlers._checks.put = function(data, callback){
    const checkId = typeof data.payload.checkId === 'string' && data.payload.checkId.trim().length > 10 ? data.payload.checkId : false;
    const protocol = typeof data.payload.protocol === 'string' && ['https', 'http'].indexOf(data.payload.protocol) > -1 ? data.payload.protocol : false;
    const url = typeof data.payload.url === 'string' && data.payload.url.trim().length > 0 ? data.payload.url.trim() : false;
    const method = typeof data.payload.method === 'string' && ['post', 'get', 'put', 'delete'].indexOf(data.payload.method) > -1 ? data.payload.method : false;
    const successCodes = typeof data.payload.successCodes === 'object' && data.payload.successCodes instanceof Array && data.payload.successCodes.length > 1 ? data.payload.successCodes : false;
    ...
    if(checkId){
        if(protocol || url || method || successCodes || timeoutSeconds){
                    ...
                    if(token){
                        handlers._tokens.verifyToken(token, checkData.phone, function(tokenIsValid){
                            if(tokenIsValid){
                                ...
                                //updating check
                                _data.put('checks', checkId, checkData, function(err){
                                    if(!err){
                                        callback(200);
                                    }else{
                                        callback(500);
                        ...
                        callback(403, {'error' : 'unauthorized access. no token provided'})
                    }
                }else{
                    callback(500)
                }
            ...
            callback(403, {'error' : 'No data provided to for update'})
        }

    }else{
        callback(400, {'error' : 'please provide the required field'})
    }
...
handlers._checks.delete = function(data, callback){
    const checkId = typeof data.queryString.checkId === 'string' && data.queryString.checkId.trim().length > 10 ? data.queryString.checkId.trim() : false;
    if(checkId){
        _data.read('checks', checkId, function(err, checkData){
            if(!err && checkData){
                ...
                handlers._tokens.verifyToken(token, checkData.phone, function(tokenIsValid){
                    if(tokenIsValid){
                        //removing check record from user data
                        const userPhone = checkData.phone;
                        _data.read('users', userPhone, function(err, userData){
                            if(!err && userData){
                                const userChecks = typeof userData.checks === 'object' && userData.checks instanceof Array && userData.checks.length > 0 ? userData.checks : [];
                                const checkPosition = userData.checks.indexOf(checkId) > -1 ? userData.checks.indexOf(checkId) : false;
                                userChecks.splice(checkPosition, 1);
                                //updating user object
                                _data.put('users', userData.phone, userData, function(err){
                                    if(!err){
                                        //deleting the check object from the check folder
                                        _data.delete('checks', checkId, function(err){
                                            if(!err){
                                                callback(200);
                                            }else{
                                                callback(500, {'error' : 'unable to delete check object'})
                                            }
                                        })
                                    }else{
                                        callback(500, {'error' : 'unable to update user check after removing specified checkId'})
                                    }
                                })
                            }else{
                                callback(500, {'error' : 'unable to read check user object'})
                            }
                        })
                    }else{
                ...
                callback(403, {'error' : 'invalid checkId - not found'})
            }
        });
    }else{
        callback(403, {'error' : 'please provide all requirement fields'})
    }
};
